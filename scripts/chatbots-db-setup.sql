-- Chatbots SaaS Platform Database Schema
...
-- 2. Create chatbot_types table (admin-controlled bot types)
...
CREATE POLICY "Users can read own profile" ON public.chatbot_users
...
-- RLS Policies for chatbot_types (everyone can read, only admin can modify)
CREATE POLICY "Everyone can read chatbot types" ON public.chatbot_types
  FOR SELECT USING (true);

CREATE POLICY "Only service role can insert chatbot types" ON public.chatbot_types
  FOR INSERT WITH CHECK (auth.role() = 'service_role');

CREATE POLICY "Only service role can update chatbot types" ON public.chatbot_types
  FOR UPDATE USING (auth.role() = 'service_role') WITH CHECK (auth.role() = 'service_role');

-- RLS Policies for deployed_bots
CREATE POLICY "Users can read own deployed bots" ON public.deployed_bots
  FOR SELECT USING (
    user_id = (SELECT id FROM public.chatbot_users WHERE auth_id = auth.uid())
  );

CREATE POLICY "Users can insert own deployed bots" ON public.deployed_bots
  FOR INSERT WITH CHECK (
    user_id = (SELECT id FROM public.chatbot_users WHERE auth_id = auth.uid())
  );

CREATE POLICY "Users can update own deployed bots" ON public.deployed_bots
  FOR UPDATE USING (
    user_id = (SELECT id FROM public.chatbot_users WHERE auth_id = auth.uid())
  ) WITH CHECK (
    user_id = (SELECT id FROM public.chatbot_users WHERE auth_id = auth.uid())
...
CREATE POLICY "Users can delete own deployed bots" ON public.deployed_bots
  FOR DELETE USING (
    user_id = (SELECT id FROM public.chatbot_users WHERE auth_id = auth.uid())
  );
...
CREATE INDEX IF NOT EXISTS idx_deployed_bots_bot_type_id ON public.deployed_bots(bot_type_id);
